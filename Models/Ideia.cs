...
namespace WebApp.Models
{
    public class Ideia
    {
        ...
        public long CadastrarIdeia()
        {
            ...
            try
            {
                try
                {
                    ...
                    // Valida usabilidade do Contrato
                    // True => pode adicionar
                    if (VerificaContratoIdeias(Convert.ToInt32(HttpContext.Current.Session["master_idUsuario"])))
                    {
                        // PROJETO
        ...
        public int BuscarQntdCards(string idUser)
        {
            var db = new ClassDb();
            int contador = 0;
            try
            {
                try
                {
                    ...
                    var sql = " select distinct a.id                                                                           " +
                              ...
                              "  inner join avaliacao_ideias c          on c.id_projeto = a.id                                 " +
                              "  inner join questoes_avaliacao_ideias d on d.id_questao_ideias = c.id_questao_avaliacao_ideias " +
                              "  inner join questoes_ideias e           on e.id = d.id_questao_ideias                          " +
                              "  where b.id = @usuario                                                                         " +
                              "  group by a.id,                                                                                " +
                              "              a.nome_projeto,                                                                   " +
                              "              e.letra,                                                                          " +
                              "              e.ordem                                                                           " +
                              "  order by a.nome_projeto,                                                                      " +
                              "              e.ordem                                                                           ";
        ...
        public List<IdeiaCards> BuscarInfoCards(string idUser)
        {
            var db = new ClassDb();
            ...
            try
            {
                try
                {
                    ...
                    while (dr.Read())
                    {
                        ...
                        item.PONTUACAO = dr["PONTUACAO"].ToString();
            ...
            try
            {
                try
                {
                    var sql = " select q.id id_questao_avaliacao_ideia, " +
                                ...
                                " from questoes_avaliacao_ideias q";
